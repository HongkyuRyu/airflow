...
with DAG(
    ...
    def check_api_update(http_conn_id, endpoint, base_dt_col, **kwargs):
        ...
        if last_date >= today_ymd:
            print(f"생성 확인(배치날짜: {today_ymd} / API LAST날짜: {last_date} )")
            return True
        else:
            print(f"Update 미완료 (배치날짜: {today_ymd} / API LAST날짜:{last_date} ")
            return False
    sensor_task = PythonSensor(
        ...
        op_kwargs={
            ...
            'endpoint': '{{var.value.apikey_openapi_seoul_go_kr}}/json/tbElectricCharging'
            # 잠깐1 실시간 데이터가 아니라서 다시 구해야한다..
        },
        # poke_interval: 
